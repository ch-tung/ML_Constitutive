...
def GapScore_cdfsq(Stress_points,quantileinf=0.5,strcoeffinf=1.,quantilesup=0.99,strcoeffsup=1.1,tol=1e-14):
    A=SortCut5(Stress_points,quantileinf=quantileinf,strcoeffinf=strcoeffinf,quantilesup=quantilesup,strcoeffsup=strcoeffsup)
    THETAS_0=np.sort(np.arctan2(A[:,1],A[:,0]))
    theta_bar = np.arctan2(np.mean(np.sin(THETAS_0)),np.mean(np.cos(THETAS_0)))
    THETAS = np.sort((THETAS_0-theta_bar+np.pi)%(2*np.pi))
    ...
    if N<=1:
        return 1
    N_bins = 1000
    theta_bins = np.arange(N_bins)/N_bins*2*np.pi
    cdf_bins = [np.sum(THETAS<theta_bins[i])/N for i in np.arange(N_bins)]
    cdf_uni_bins = theta_bins/2/np.pi
    r_bins = (cdf_bins-cdf_uni_bins)**2
    return np.sum(r_bins)/N_bins*(2*np.pi)/(np.pi/6)
